...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ user }) => {
  ...
  // Menggunakan useToggle untuk sidebar
  ...
  const renderActiveView = () => {
    switch (activeView) {
      ...
      case 'approval':
        return user.role === 'admin' ? <ApprovalView user={user} /> : (
  ...
  return (
    <div className="flex w-screen h-screen bg-gray-100">
      ...
      <div className="flex-1 overflow-auto bg-gray-50">
        {isClient ? renderActiveView() : <div className="flex items-center justify-center h-full"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-500"></div></div>}
      </div>
    </div>
  );
};
