...
const Page = () => {
  ...
  const validateForm = () => {
    ...
    if (activeForm !== 'forgotPassword' && (!hasUppercase || !hasLowercase || !hasNumber || !hasSpecialChar || password.length < 8)) {
      isValid = false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateForm()) {
      await Swal.fire({
        ...
        text: 'Please fill in all required fields correctly. Password must be at least 8 characters long and include uppercase, lowercase, number, and special character.',
      });
      return;
    }
        ...
        <div className="flex flex-1 overflow-hidden">
                    ...
                    <Input label="Phone Number *" error={phoneNumberError}>
                      <div className="flex">
                        <select
                          value={countryCode}
                          onChange={(e) => setCountryCode(e.target.value)}
                          className="flex-shrink-0 px-3 py-2 sm:py-2 md:py-3 text-sm text-gray-900 bg-gray-200 border border-r-0 border-gray-300 rounded-l-md focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-transparent"
                        >
                          {countryCodes.map((c) => (<option key={c.code} value={c.code}>{c.abbr} {c.code}</option>))}
                        </select>
                        <Input
                          ...
                          onChange={(e) => setPhoneNumber(e.target.value)}
                          placeholder="812-3456-7890"
                          className="w-full p-2 sm:p-2 md:p-3 text-sm text-gray-900 border border-gray-200 rounded-r-md focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-transparent"
                        />
                      </div>
                    </Input>
          ...
          <div className="w-full md:w-3/5 bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4 sm:p-6 md:p-8">
            <div className="relative w-full h-40 sm:h-48 md:h-64 lg:h-80 xl:h-full max-w-xs sm:max-w-sm md:max-w-lg">
              <Image
                src={activeForm === 'login' ? '/images/login_form.png' : activeForm === 'register' ? '/images/register_form.png' : '/images/forgot_password.png'}
                alt={activeForm === 'login' ? 'Login Illustration' : activeForm === 'register' ? 'Register Illustration' : 'Forgot Password Illustration'}
                fill
